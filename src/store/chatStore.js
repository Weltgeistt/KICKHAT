...
function sanitizeSettings(s) {
  return {
    ...
    games_active: s.games_active !== undefined ? !!s.games_active : DEFAULT_SETTINGS.games_active,
  };
}
...
// ── XP ve Seviye Veritabanı (Yerel) ───────────────────────────────────────
const XP_KEY = 'kickhat:xp_data';
...
function persistXpData(xpData) {
  try { localStorage.setItem(XP_KEY, JSON.stringify(xpData)); } catch { /* ignore */ }
}
...
export const useChatStore = create((set, get) => ({
  ...
  // ── XP ve Gamification State ──────────────────────────────────────
  userXp: loadXpData(),

  awardXp: (username) => {
    const { settings, userXp, addToast } = get();
    if (!settings.games_active) return;

    set((state) => {
      const currentData = state.userXp[username] || { xp: 0, level: 1 };

      // Rastgele 1 ile 3 XP
      const gained = Math.floor(Math.random() * 3) + 1;
      let newXp = currentData.xp + gained;
      let newLevel = currentData.level;

      // Seviye atlama kuralı (Örn: level başına (level * 100) XP)
      const requiredForNext = newLevel * 100;
      let leveledUp = false;

      if (newXp >= requiredForNext) {
        newLevel += 1;
        newXp = newXp - requiredForNext; // Artan XP bir sonraki levele kalsın
        leveledUp = true;
      }

      const newData = { ...state.userXp, [username]: { xp: newXp, level: newLevel } };
      persistXpData(newData);

      if (leveledUp) {
        setTimeout(() => {
          get().addToast(`🎉 ${username} seviye atladı! Yeni seviyesi: ${newLevel}`, 'success');
        }, 0);
      }

      return { userXp: newData };
    });
  },
...
2. Robotik girişler ("Görevin..." vb.) yapma, doğrudan muhabbete girer gibi anlatmaya başla.`;
  ...
  startCustomResearch: async (query) => {
    ...
    const langName = i18n.language === 'tr' ? 'TÜRKÇE' : (i18n.language === 'en' ? 'ENGLISH' : (i18n.language === 'es' ? 'ESPAÑOL' : 'ITALIANO'));
    const promptText = `Yayıncı şu an yayında ve "${query}" konusunu araştırıyor. İnternetteki son arama sonuçları aşağıdadır:
${searchContext}

GÖREVİN: 
1. ÖNCE, yayıncının sorduğu konu hakkında bilgi ver. DİKKAT: Eğer sana verdiğim arama sonuçları kullanıcının sorduğu "${query}" konusuyla ALAKASIZ ise, o arama sonuçlarını TAMAMEN GÖRMEZDEN GEL ve hiçbir şekilde onlardan bahsetme! Sadece kendi yapay zeka bilgini kullanarak doğru cevabı ver.
2. ANALİZ VE GELECEK ÖNGÖRÜSÜ: Eğer konunun gidişatı ile ilgili bir analiz gerekiyorsa (oyun çıkış tarihi vb.), gerçek verilere ve mantığa dayanarak akıl yürüt.
3. EN SONDA, yayıncının bu konuyu yayında chat ile nasıl tartışabileceğine dair 1-2 maddelik kısa bir "Yayın Taktiği" ver.

KESİN KURALLAR:
1. SADECE ${langName} YAZ. BAŞKA DİLDE YAZARSAN SİSTEM ÇÖKER. (ONLY WRITE IN ${langName}).
2. Robotik girişleri bırak, doğrudan konuya gir.`;

    try {
      const { invoke: inv3 } = await import('@tauri-apps/api/core');
      const { settings } = get();

      const sysPrompt = `Sen Twitch/Kick yayıncılarına bilgi ve taktik veren zeki, analitik düşünen bir asistansın. Gelecek öngörülerini gerçeklere dayandırırsın. ASLA BAŞKA BİR DİL KONUŞMA. Sadece ${langName} yaz. (ONLY RESPOND IN ${langName})`;
  ...
  generateDetailedAnalysis: async (analysisId) => {
    ...
    const langName = i18n.language === 'tr' ? 'TÜRKÇE' : (i18n.language === 'en' ? 'ENGLISH' : (i18n.language === 'es' ? 'ESPAÑOL' : 'ITALIANO'));
    const promptText = `Aşağıda "Güncel Haber ve Gündemi" özetleri yer almaktadır:${queryContextText}
...
KURALLAR:
1. YANITINI KESİNLİKLE VE SADECE ${langName} OLARAK YAZACAKSIN. Yabancı dilde tek bir kelime bile yazma! (ONLY WRITE IN ${langName})
2. ASLA YENİ HABER UYDURMA. Sadece sana verilen haber özetini kullan. 
...
LÜTFEN SADECE VE SADECE ${langName} OLARAK, DOĞRUDAN STRATEJİYİ YAZ (Başka hiçbir açıklama yapma):`;

    try {
      const { invoke: inv5 } = await import('@tauri-apps/api/core');
      const { settings } = get();

      const sysPrompt = `Sen sadece ${langName} konuşan, yayıncılara taktik veren profesyonel bir stratejistsin. Asla başka bir dilde cevap vermezsin. (ONLY SPEAK ${langName})`;
  ...
  runAiAnalysis: async (customQuery = null, timeframe = null) => {
    ...
    try {
      ...
      const langName = i18n.language === 'tr' ? 'TÜRKÇE' : (i18n.language === 'en' ? 'ENGLISH' : (i18n.language === 'es' ? 'ESPAÑOL' : 'ITALIANO'));
      const noNewsText = i18n.language === 'tr' ? 'Şu an gündemde öne çıkan bir haber yok.' : (i18n.language === 'en' ? 'There is no prominent news on the agenda right now.' : 'No hay noticias destacadas en la agenda en este momento.');
      const noIssuesText = i18n.language === 'tr' ? 'Sorun yok' : (i18n.language === 'en' ? 'No issues' : 'Sin problemas');

      const promptJson = `Aşağıda bir yayıncının canlı chat kayıtları ve anlık internet gündemi (RSS haberleri) bulunmaktadır. Sen profesyonel bir yayın asistanısın.

GÖREVİN: Chat mesajlarını ve İnternet Gündemini analiz edip aşağıdaki 4 kategoride tam ve eksiksiz bir JSON formatında özet çıkarmaktır. BÜTÜN CEVAPLARINI ${langName} DİLİNDE VERECEKSİN. (ALL JSON VALUES MUST BE WRITTEN IN ${langName})

KURALLAR:
1. questions: SADECE "Chat Mesajları" bölümündeki izleyici mesajlarına bakarak yayıncıya sorulan gerçek soruları listele. Gerçek bir izleyici sorusu yoksa boş bırak: []
2. topics: "Chat Mesajları" kısmında chat şu an ağırlıklı olarak ne konuşuyor? Sadece en çok konuşulan 1 veya 2 konuyu çok kısa (1-2 kelime) yaz.
3. technical_issues: Eğer "Chat Mesajları" kısmında yayında "kasıyor", "dondu" gibi net bir şikayet varsa yaz. Yoksa sadece ["${noIssuesText}"] yaz.
4. "news_summary" ALANI: 
- EĞER kullanıcının özel bir sorusu veya araması varsa ("${customQuery}"): BU ALANA SADECE O SORUNUN DOĞRUDAN CEVABINI YAZ (Örn: "news_summary": "Türkiye bayrağının renkleri kırmızı ve beyazdır."). Verilen haberler soruyla alakasızsa onları tamamen yok say.
- EĞER özel arama yoksa: İnternet Gündemi'ni özetle. Gündem yoksa "${noNewsText}" yaz.

ÇOK ÖNEMLİ: Çıktın SADECE geçerli bir JSON olmalıdır. Başka hiçbir metin ekleme. JSON anahtarları tam olarak: "questions", "topics", "technical_issues", "news_summary" olmalıdır.
...
Beklenen JSON Formatı:
{
  "questions": ["soru 1"],
  "topics": ["konu başlığı"],
  "technical_issues": ["${noIssuesText}"],
  "news_summary": "Buraya internet gündeminin özeti veya sorulan sorunun doğrudan cevabı gelecek..."
}`;
